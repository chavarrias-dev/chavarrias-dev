import Link from "next/link";
import { notFound, redirect } from "next/navigation";
import type { ClientOption } from "@/components/clients/types";
import { EditPedimentoForm } from "@/components/pedimentos/edit-pedimento-form";
import { getUserRole } from "@/lib/supabase/middleware";
import { createSupabaseServerClient } from "@/lib/supabase/server";

type PedimentoRecord = {
  id: string;
  numero_pedimento: string;
  fecha: string;
  aduana: string;
  cliente_id: string | null;
  archivo_url: string | null;
  notas: string | null;
};

type PageProps = {
  params: Promise<{ id: string }>;
  searchParams: Promise<{ error?: string }>;
};

export default async function EditPedimentoPage({
  params,
  searchParams,
}: PageProps) {
  const { id } = await params;
  const supabase = await createSupabaseServerClient();
  const {
    data: { user },
  } = await supabase.auth.getUser();
  if (!user) {
    redirect("/login");
  }

  const authRole = await getUserRole(supabase, user.id);
  const isAdmin = authRole === "admin";

  const { data: pedimento, error } = await supabase
    .from("pedimentos")
    .select(
      "id, numero_pedimento, fecha, aduana, cliente_id, archivo_url, notas",
    )
    .eq("id", id)
    .maybeSingle();

  if (error || !pedimento) {
    notFound();
  }

  const p = pedimento as PedimentoRecord;

  const { data: clientRows } = await supabase
    .from("clients")
    .select("id, full_name")
    .order("full_name", { ascending: true });

  let clients = (clientRows ?? []) as ClientOption[];
  if (p.cliente_id && !clients.some((c) => c.id === p.cliente_id)) {
    const { data: linked } = await supabase
      .from("clients")
      .select("id, full_name")
      .eq("id", p.cliente_id)
      .maybeSingle();
    if (linked) {
      clients = [linked as ClientOption, ...clients];
    }
  }

  if (clients.length === 0) {
    return (
      <main className="font-poppins w-full flex-1 px-6 py-8 lg:px-10">
        <div className="mx-auto max-w-xl rounded-2xl border border-amber-200/90 bg-amber-50/80 px-5 py-8 text-center">
          <p className="text-sm text-amber-950">
            Necesitas al menos un cliente asignado.
          </p>
          {isAdmin ? (
            <Link
              href="/dashboard/users/new"
              className="mt-4 inline-flex text-sm font-medium text-[#227DE8] underline-offset-2 hover:underline"
            >
              Registrar usuario cliente
            </Link>
          ) : (
            <span className="mt-4 block text-sm text-slate-600">
              Pide a un administrador que registre clientes en el sistema.
            </span>
          )}
        </div>
      </main>
    );
  }

  const sp = await searchParams;
  const errorMessage = sp.error
    ? decodeURIComponent(sp.error)
    : undefined;

  return (
    <main className="w-full flex-1 px-6 py-8 lg:px-10">
      <EditPedimentoForm
        pedimento={{
          id: p.id,
          cliente_id: p.cliente_id,
          numero_pedimento: p.numero_pedimento,
          fecha: p.fecha,
          aduana: p.aduana,
          notas: p.notas,
          archivo_url: p.archivo_url,
        }}
        clients={clients}
        errorMessage={errorMessage}
      />
    </main>
  );
}
